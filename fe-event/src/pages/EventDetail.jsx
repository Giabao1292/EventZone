...
import ReviewSection from "../components/review/ReviewSection"; // Kiểm tra đúng đường dẫn nhé
...
const EventDetail = () => {
  ...
  // Lấy thông tin user và các suất chiếu đã mua
  useEffect(() => {
    ...
    const fetchConfirmed = async () => {
      try {
        ...
        if (token) {
          const res = await axios.get("/api/bookings/confirmed-showing-time-ids", {
  ...
  // Kiểm tra đã theo dõi event chưa
  useEffect(() => {
    if (event?.id) {
