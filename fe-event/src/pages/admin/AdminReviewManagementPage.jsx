...
function normalizeEventList(rawEvents) {
    ...
    return rawEvents.map(ev => ({
        ...
        title: ev.title || ev.eventTitle || ev.name || ev.eventName || `Sự kiện #${ev.id}`,
    }));
}
...
const AdminReviewManagementPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchEvents = async () => {
            setLoading(true);
            try {
                const data = await eventService.getAllEvents(user.token);
                // CHUẨN HÓA LIST SỰ KIỆN
                const normalizedEvents = normalizeEventList(data || []);
                setEvents(normalizedEvents);
                if (normalizedEvents.length > 0) setSelectedEvent(normalizedEvents[0].id);
    ...
    useEffect(() => {
        ...
        const fetchShowingTimesAndEventReviews = async () => {
            ...
            try {
                ...
                if (allShowingTimes.length > 0) setSelectedShowingTime(allShowingTimes[0].id);
    ...
    useEffect(() => {
        ...
        const fetchReviewsForAdmin = async () => {
            ...
            try {
                ...
                const reviewsData = Array.isArray(reviewList) ? reviewList : (reviewList.content || []);
    ...
    const eventStarData = eventStarCounts.map((item) => ({
        ...item,
        percent: totalEventReviews === 0 ? 0 : Math.round((item.count / totalEventReviews) * 100),
    }));
    ...
    const showingStarData = showingStarCounts.map((item) => ({
        ...item,
        percent: totalShowingReviews === 0 ? 0 : Math.round((item.count / totalShowingReviews) * 100),
    }));
    ...
    const handleToggleStatus = async (reviewId, status) => {
        ...
        try {
            ...
            if (selectedShowingTime) {
                const reviewList = await reviewService.getReviewsForAdmin(selectedShowingTime, page, size, "all");
                const reviewsData = Array.isArray(reviewList) ? reviewList : (reviewList.content || []);
                setReviews(reviewsData || []);
            }
        ...
        <div className="max-w-[1400px] mx-auto p-6 bg-gray-50 rounded-3xl shadow-lg">
            ...
            <section className="mb-10 bg-white p-8 rounded-2xl shadow-md">
                <h2 className="text-xl font-bold mb-6 border-b border-green-300 pb-2 text-green-800">
                    Tổng quan sự kiện:{" "}
                    <span className="text-green-600">{currentEvent?.title || "Chưa chọn sự kiện"}</span>
                </h2>
                <div className="flex flex-wrap md:items-center md:gap-12">
                    <div className="flex flex-col items-center justify-center text-center px-4 py-6 bg-green-100 rounded-xl shadow-inner flex-1 min-w-[180px]">
                        <div className="text-4xl font-bold text-green-700">{totalEventReviews}</div>
                        <div className="text-green-600 font-semibold mt-1">Tổng lượt đánh giá</div>
                    </div>
                    <div className="flex flex-col items-center justify-center text-center px-4 py-6 bg-amber-100 rounded-xl shadow-inner flex-1 min-w-[180px] mt-6 md:mt-0">
                        <div className="flex items-center gap-2 text-5xl font-extrabold text-amber-600">
                            {totalEventReviews === 0
                                ? "0.0"
                                : (eventActiveReviews.reduce((acc, r) => acc + r.rating, 0) / totalEventReviews).toFixed(1)}
                            <Star className="w-10 h-10" fill="#facc15" />
                        </div>
                        <div className="text-amber-700 font-semibold mt-1">Điểm đánh giá trung bình</div>
                    </div>

                    {/* Biểu đồ cột tổng quan event */}
                    <div className="w-full max-w-xl mt-8 md:mt-0">
                        <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={eventStarData} margin={{ top: 15, right: 20, left: 0, bottom: 15 }}>
                                <CartesianGrid strokeDasharray="3 3" />
                                <XAxis
                                    dataKey="star"
                                    tick={{ fontWeight: "bold", fontSize: 16 }}
                                    label={{ value: "Số sao", position: "insideBottom", dy: 15, fontSize: 16 }}
                                />
                                <YAxis
                                    allowDecimals={false}
                                    label={{ value: "Số lượng", angle: -90, dx: -15, position: "insideLeft", fontSize: 16 }}
                                    tick={{ fontSize: 16 }}
                                />
                                <Tooltip formatter={(value) => [`${value} đánh giá`]} />
                                <Bar dataKey="count" fill="#34d399" radius={[8, 8, 0, 0]} barSize={34} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
            ...
            <section className="mb-10 bg-white p-8 rounded-2xl shadow-md">
                ...
                <div className="flex flex-wrap gap-6 mb-8">
                    {/* Event Selector */}
                    <div className="w-64">
                        <label className="block mb-1 font-semibold text-gray-700">Sự kiện</label>
                        <select
                            value={selectedEvent ?? ""}
                            onChange={(e) => {
                    ...
                    <div className="w-64">
                        <label className="block mb-1 font-semibold text-gray-700">Suất chiếu</label>
                        <select
                    ...
                    {/* Star filter */}
                    <div className="w-40">
                        <label className="block mb-1 font-semibold text-gray-700">Đánh giá</label>
                        <select
                            value={filterStar}
                ...
                <div className="mb-8 max-w-3xl">
                    <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={showingStarData} margin={{ top: 15, right: 20, left: 0, bottom: 15 }}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis
                                dataKey="star"
                                tick={{ fontWeight: "bold", fontSize: 16 }}
                ...
                <div className="overflow-x-auto rounded-xl shadow-lg bg-white border border-green-300">
                    <table className="min-w-full table-fixed text-gray-800">
                        ...
                        <tr>
                            <th className="py-3 px-4 font-semibold text-left text-green-700">#</th>
                            <th className="py-3 px-4 font-semibold text-left text-green-700">Email</th>
                            <th className="py-3 px-4 font-semibold text-left text-green-700">Đánh giá</th>
                            <th className="py-3 px-4 font-semibold text-left text-green-700">Bình luận</th>
                            <th className="py-3 px-4 font-semibold text-center text-green-700">Ngày gửi</th>
                            <th className="py-3 px-4 font-semibold text-center text-green-700">Trạng thái</th>
                        </tr>
                        </thead>
                        <tbody>
                        {loading ? (
                            <tr>
                                <td colSpan={6} className="text-center py-12 text-gray-600 text-lg">
                                    Đang tải...
                                </td>
                            </tr>
                        ) : filteredReviews.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="text-center py-12 text-green-600 italic">
                            ...
                            filteredReviews.map((r, idx) => (
                                ...
                                >
                                    <td className="px-4 py-3 text-left font-medium">{idx + 1}</td>
                                    <td className="px-4 py-3 break-words">{r.userEmail || "--"}</td>
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        <button
                                            ...
                                            title={r.status === "active" ? "Ẩn bình luận" : "Hiện bình luận"}
                                        >
                                            {hiding[r.reviewId] ? (
                                                "..."
                                            ) : r.status === "active" ? (
