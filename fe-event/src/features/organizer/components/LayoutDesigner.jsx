...
import {
  Save, Plus, Grid3x3, Square, Trash2, ZoomIn, ZoomOut, Edit, Users,
} from "lucide-react";
...
export default function LayoutDesigner({ onSave }) {
  ...
  const showingTimeId = location.state?.showingTimeId || location.pathname.split("/").pop();
  ...
  // Thêm ghế nhanh
  const quickAddSeats = () => {
    if (!quickAddName.trim()) return showToast("Vui lòng nhập tên ghế!", "warning");
    let newSeats = [];
  ...
  // Thêm khu vực nhanh
  const addZone = () => {
    if (!quickAddName.trim()) return showToast("Vui lòng nhập tên khu vực!", "warning");
    setZones((prev) => [
      ...prev,
      {
  ...
  const deleteSelected = () => {
    setSeats((prev) => prev.filter((s) => !selectedItems.includes(`seat-${s.id}`)));
    setZones((prev) => prev.filter((z) => !selectedItems.includes(`zone-${z.id}`)));
    setSelectedItems([]);
  ...
  const addNewSeatType = () => {
    ...
    setNewSeatType({ name: "", color: "bg-emerald-500", price: 80000, capacity: 1 });
    showToast(`Đã thêm loại "${newSeatType.name}"!`, "success");
  };

  const getSeatTypeData = (typeName) => seatTypes.find((t) => t.name === typeName) || seatTypes[0];
