...
import { updateShowingTime, fetchEventById } from "../../services/showingTime";
...
const EditEventForm = () => {
    const { id: eventId } = useParams(); // lấy eventId từ URL, FE route phải là /organizer/edit-event/:id
    ...
    // Lấy data sự kiện theo eventId
    useEffect(() => {
        const fetchEvent = async () => {
            try {
                const res = await fetchEventById(eventId); // Gọi API của bạn, trả về dữ liệu đầy đủ
                setEventData({
                    ...res,
                    event_title: res.eventTitle,
                    category_id: res.category?.categoryId?.toString() ?? "",
                    description: res.description,
                    age_rating: res.ageRating,
                    banner_text: res.bannerText,
                    header_image: res.headerImage,
                    poster_image: res.posterImage,
                    start_time: res.startTime,
                    end_time: res.endTime,
                    location: res.location ?? "",
                    city: res.city ?? "",
                    hasDesignedLayout: !!res.hasDesignedLayout,
                    venueName: res.venueName ?? "",
                    max_capacity: res.maxCapacity ?? "",
                    status_id: res.statusId ?? 1,
                    showingTimes: res.showingTimes ?? [],
                    id: res.id,
                });
            } catch (err) {
    ...
    const handleNextStep = async () => {
        ...
        try {
            ...
            if (currentStep === 2) {
                ...
                await updateShowingTime(payload); // Bạn tự implement hoặc import đúng API
                toast.success("Cập nhật thời gian & địa điểm thành công!");
            }
            if (currentStep === 4) {
                // Submit để duyệt lại
                await apiClient.post(`/events/submit/${eventId}`);
                toast.success("Sự kiện đã được gửi lại để phê duyệt!", {
                    autoClose: 2000,
            ...
            toast.error(
                err?.message || "Có lỗi xảy ra khi cập nhật. Vui lòng kiểm tra lại."
            );
        } finally {
            setLoading(false);
        }
    ...
    const renderStepContent = () => {
        if (!eventData) return <div>Đang tải dữ liệu...</div>;
        const stepProps = {
            eventData,
            handleInputChange,
            categories,
        ...
        switch (currentStep) {
            ...
            case 4:
                return <div>Thông tin thanh toán chưa làm, bấm hoàn tất để lưu</div>;
            default:
                return null;
        }
    };
