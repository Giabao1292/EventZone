...
export default function EventManager() {
    ...
    useEffect(() => {
        ...
        Promise.all(
            STATUS_TABS.map(tab =>
                getEventsByStatus(organizerId, tab.id).then(list => [tab.id, list.length])
            )
    ...
    function EmptyIcon() {
        if (currentTab === 3) return <HiOutlineXMark className="text-5xl text-red-400 mb-2" />;
        if (currentTab === 4) return <HiOutlineCheck className="text-5xl text-green-400 mb-2" />;
        if (currentTab === 2) return <HiOutlineClock className="text-5xl text-yellow-400 mb-2" />;
        return <HiOutlineDocument className="text-5xl text-gray-400 mb-2" />;
    }

    return (
        <div className="p-4 sm:p-8 max-w-7xl mx-auto">
            <h1 className="text-2xl sm:text-3xl font-bold mb-8 text-white">🎫 Quản lý sự kiện của bạn</h1>
                    ...
                    {event.status === "DRAFT" && "Nháp"}
                                ...
                                <div className="flex-1 flex flex-col gap-2 p-5">
                                    <div className="flex items-center gap-2">
                                        <span className="font-bold text-lg text-white truncate">{event.title}</span>
                                        <span className="ml-auto px-2 py-0.5 rounded bg-blue-900/80 text-xs text-blue-200 font-medium">{event.category}</span>
                                    </div>
                                    ...
                                    <div className="text-sm text-gray-300 mt-1 line-clamp-2">{event.description}</div>
                                    ...
                                    {event.status === "REJECTED" && event.rejectionReason && (
                                        <div className="flex items-center gap-1 mt-2 text-xs text-red-300 bg-red-900/40 px-3 py-2 rounded">
                                            <HiOutlineInformationCircle /> Lý do từ chối: {event.rejectionReason}
                                        </div>
                                    )}
                                    <Link
