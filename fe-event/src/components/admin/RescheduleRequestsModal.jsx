...
export default function RescheduleRequestsModal({ eventId, onClose, adminUserId }) {
    ...
    const handleReject = async () => {
        ...
        setActionLoading(true);
        try {
            await eventRescheduleService.rejectRequest(selectedRejectId, adminUserId, rejectNote);
    ...
    const filteredRequests = selectedShowingTime
        ? requests.filter(r => String(r.showingTimeId) === String(selectedShowingTime))
        : [];
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
            <div className="relative bg-white rounded-lg max-w-2xl w-full p-0 shadow-xl">
                ...
                <div className="overflow-auto p-6 max-h-[70vh]">
                    ...
                    <div>
                        <div className="mb-3 font-medium">Danh sách suất chiếu có yêu cầu:</div>
                        <div className="flex gap-2 flex-wrap mb-4">
                            {showingTimes.length === 0 && (
                                <span className="text-gray-500">Không có suất chiếu nào có yêu cầu dời lịch.</span>
                            )}
                            {showingTimes.map(st => (
                                <button
                                    key={st.id}
                                    className={`px-3 py-1 rounded ${selectedShowingTime === st.id
                                        ? "bg-blue-600 text-white"
                                        : "bg-blue-100 text-blue-700 hover:bg-blue-200"
                                    } relative flex items-center`}
                                    onClick={() => setSelectedShowingTime(st.id)}
                                >
                                    {new Date(st.oldStartTime).toLocaleString()} - {new Date(st.oldEndTime).toLocaleString()}
                                    <span className="ml-2 bg-red-600 text-white rounded-full px-2 py-0.5 text-xs font-semibold">{st.count}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    {selectedShowingTime && (
                        <ul>
                            {filteredRequests.length === 0 ? (
                                <div className="text-gray-500 mb-2">Chưa có yêu cầu nào cho suất chiếu này.</div>
                            ) : (
                                filteredRequests.map(req => (
                                    <li key={req.id} className="border p-3 rounded mb-3">
                                        <p><b>Thời gian cũ:</b> {new Date(req.oldStartTime).toLocaleString()} - {new Date(req.oldEndTime).toLocaleString()}</p>
                                        <p><b>Thời gian yêu cầu:</b> {new Date(req.requestedStartTime).toLocaleString()} - {new Date(req.requestedEndTime).toLocaleString()}</p>
                                        <p><b>Lý do:</b> {req.reason}</p>
                                        <b className="text-red-700">Lý do từ chối:</b> <span className="text-red-600">{req.adminNote || "(Không ghi chú)"}</span>
                                        <p><b>Trạng thái:</b> {req.status}</p>
                                            ...
                                            <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded">
                                                ...
                                                {req.approvedByName && (
                                                    <span className="text-xs text-gray-500">
                                                        Từ chối bởi: {req.approvedByName} {req.respondedAt ? `- ${new Date(req.respondedAt).toLocaleString()}` : ""}
                                                    </span>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {req.status === "APPROVED" && req.approvedByName && (
                                            <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded">
                                                <b className="text-green-700">Đã duyệt bởi:</b> <span className="text-green-600">{req.approvedByName}</span>
                                                {req.respondedAt && (
