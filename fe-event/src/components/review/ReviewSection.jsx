...
import { Star, MessageSquareText, Smile, Edit, Trash2, X, Check, CornerDownLeft } from "lucide-react";
...
const ReviewSection = ({ showingTimeId, canReview }) => {
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            Object.entries(replyEmojiPickerRefs.current).forEach(([reviewId, ref]) => {
    ...
    const addReplyEmoji = (reviewId, emoji) => {
        setReplyContent((prev) => ({ ...prev, [reviewId]: (prev[reviewId] || "") + emoji.native }));
        setShowReplyEmoji((prev) => ({ ...prev, [reviewId]: false }));
    };
                ...
                <div className="px-14 pt-8 pb-8 flex flex-col gap-5 max-h-[520px] overflow-y-auto bg-white">
                    {loading ? (
                        ...
                        filteredReviews.map((r, idx) => (
                            <div key={r.reviewId} className="flex gap-5 items-start bg-white rounded-2xl shadow border border-gray-100 px-6 py-4">
                                {/* Avatar + bình luận của user */}
                                <div className={`flex-shrink-0 w-14 h-14 rounded-full flex items-center justify-center text-2xl font-bold text-white shadow ${getAvatarColor(idx)}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap mb-1">
                                        <span>
                                            {Array(r.rating).fill().map((_, i) => (
                                                <Star key={i} className="inline w-5 h-5 text-yellow-400" fill="#fde047" />
                                            ))}
                                        </span>
                                    ...
                                    {editId === r.reviewId ? (
                                        <form className="flex flex-col gap-2 mt-2" onSubmit={(e) => handleEditSubmit(e, r.reviewId)}>
                                            <div className="flex items-center gap-3">
                                                <StarRating value={editRating} onChange={setEditRating} size={28} color="#fde047" />
                                            </div>
                                            <textarea
                                            ...
                                            <div className="flex gap-3 justify-end">
                                                <button type="button" className="flex items-center gap-1 px-4 py-2 text-base font-semibold rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200" onClick={cancelEdit}>
                                                    <X className="w-5 h-5" /> Hủy
                                                </button>
                                                <button type="submit" className="flex items-center gap-1 px-4 py-2 text-base font-semibold rounded-full bg-emerald-500 text-white shadow hover:bg-emerald-600 transition"
                                    ...
                                    <div className="ml-8 mt-3">
                                            ...
                                            {replies[r.reviewId] && replies[r.reviewId].length > 0 && (
                                                replies[r.reviewId].map(reply =>
                                                    editReplyId === reply.id ? (
                                                        // Edit reply form
                                                        <form key={reply.id} className="flex flex-col gap-2 mb-2">
                                                            <textarea
                                                        ...
                                                        <div key={reply.id} className="flex items-start gap-2 bg-emerald-50 rounded-xl p-3 mb-1 border border-emerald-200">
                                                            <div className="flex-shrink-0 w-10 h-10 rounded-full bg-emerald-200 flex items-center justify-center font-bold text-emerald-700">O</div>
                                                            <div className="flex-1">
                                                                ...
                                                                <div className="text-xs text-gray-400 mt-1">{reply.createdAt && new Date(reply.createdAt).toLocaleString("vi-VN")}</div>
                                                            </div>
                                                            {user && (user.organizer?.id ?? user.organizerId) === reply.organizerId && (
                                                                <div className="flex flex-col gap-1 ml-2">
                                                                    ...
                                                                    <button className="text-red-500" title="Xoá" onClick={() => handleDeleteReply(reply, r.reviewId)}>
                                                                        <Trash2 size={18} />
                                                                    </button>
                                                                </div>
                                                            )}
                                        ...
                                        {user && (user.roles?.includes("ORGANIZER") || user.role === "ORGANIZER") && showReplyBox[r.reviewId] && (
                                            <form className="flex items-end gap-2 mt-3 sticky bottom-0 bg-white z-20 pt-2 pb-1"
                                                  onSubmit={e => {
                                                      e.preventDefault();
                                                ...
                                                <div className="relative flex-1">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => setShowReplyEmoji(prev => ({ ...prev, [r.reviewId]: !prev[r.reviewId] }))}
                                                        tabIndex={-1}
                                                    ...
                                                    {showReplyEmoji[r.reviewId] && (
                                                        <div
                                                            ref={el => replyEmojiPickerRefs.current[r.reviewId] = el}
                                                            className="absolute right-0 bottom-14 z-50"
                                                        >
                                                            <Picker
                                                                data={data}
                                                                onEmojiSelect={emoji => addReplyEmoji(r.reviewId, emoji)}
                                                                theme="light"
                                                            />
                                                        </div>
                                                    )}
                                                ...
                                                <button type="button" className="flex items-center gap-1 px-4 py-2 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200" onClick={() => toggleReplyBox(r.reviewId)}>
                                                    <X className="w-5 h-5" /> Hủy
                                                </button>
                                            </form>
                                        )}
                                        {!showReplyBox[r.reviewId] && user && (user.roles?.includes("ORGANIZER") || user.role === "ORGANIZER") && (
                                            <button
